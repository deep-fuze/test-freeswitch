...
char *sip_header_as_string(su_home_t *home, sip_header_t const *h)
{
  ...
  if (len == -1)
    ...
    /*
     * [Raghu] When MSG_STRING_E is the last encoding instruction, then there is a issue
     * of len being less than 1 byte needed. This is due to the bug in the MSG_STRING_E()
     * where it checks for atleast _n+1 bytes room, but increments ptr by only _n bytes.
     * If we try to fix MSG_STRING_E, then it might break existing encoding functions which
     * don't terminate using MSG_TERM_E() (for ex. sip_reason_e()). So lets allocate
     * one more byte extra to compensate for this descrepancy.
     * Also check for these two extra bytes at other places as well.
     */
    len += 2;
  }
  ...
  for (rv = su_alloc(home, len);
    ...
    if (n > -1 && n + 2 <= len) {
      rv[n] = '\0'; /* [Raghu]: For encode functions that don't do MSG_TERM_E()*/
      break;
    }
    if (n > -1)			/* glibc >2.1 */
      len = n + 2;
