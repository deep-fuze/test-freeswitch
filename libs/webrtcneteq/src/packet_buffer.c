                                 ...
                                 int16_t *pw16_memory, int memorySize)
{
    ...
    bufferInst->maxNumPacketsInBuffer = 0;
                                   ...
                                   int16_t *flushed, int av_sync)
{
    ...
    *flushed = 0;
    ...
    bufferInst->numPacketsInBuffer++;
    if (bufferInst->numPacketsInBuffer > bufferInst->maxNumPacketsInBuffer)
		bufferInst->maxNumPacketsInBuffer = bufferInst->numPacketsInBuffer;
    bufferInst->currentMemoryPos += (RTPpacket->payloadLen + 1) >> 1;
