...
#include "interface/neteq_statistics.h"
...
typedef struct
{
    ...
    uint32_t consecutive_non_accelerates;
} MCUInst_t;
 ...
 *	- last_seq_no   : highest sequence number read so far.
 *	- consecutive_lost: consecutive packets lost since last read from packet buffer
 *	- total_decoded: Total packets that are picked up for decoding so far.
 * Return value         :  0 - Ok
 *                        <0 - Error
 */
int WebRtcNetEQ_SignalMcu(MCUInst_t *inst, uint16_t *last_seq_no, 
                        uint16_t *consecutive_lost, uint32_t *total_decoded);
 ...
 *		- flushed 	: set to 1 if packet buffer was flushed now
 * Return value			:  0 - Ok
 *						  -1 - Error
 */

int WebRtcNetEQ_RecInInternal(MCUInst_t *MCU_inst, RTPPacket_t *RTPpacket,
                              uint32_t uw32_timeRec, int16_t *flushed);
