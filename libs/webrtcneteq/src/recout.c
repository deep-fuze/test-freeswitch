                               ...
                               int av_sync)
{
    ...
    switch (instr)
    {
	    ...
	    int ts_mult = inst->millisecondsPerCall / 10;
            if (len < 3 * 80 * fs_mult)
            {
                ...
                borrowedSamples = 3 * 80 * fs_mult * ts_mult - len;
...
#endif
                    pw16_decoded_buffer, 30 * (inst->timestampsPerCall / inst->millisecondsPerCall),
                    pw16_NetEqAlgorithm_buffer, &len, BGNonly);
                ...
                if (len < borrowedSamples)
                {
                    ...
                    temp_var = 30 * (inst->timestampsPerCall / inst->millisecondsPerCall) - len;
                    if (fwrite(&temp_var, sizeof(int), 1, delay_fid2) != 1) {
                      return -1;
                    }
#endif
        ...
        case DSP_INSTR_PREEMPTIVE_EXPAND:
            if (len < 30 * (inst->timestampsPerCall / inst->millisecondsPerCall))
            {
                /* borrow samples from sync buffer if necessary */
                borrowedSamples = 30 * (inst->timestampsPerCall / inst->millisecondsPerCall) - len; /* borrow this many samples */
    ...
    /*
     * Double-check that we actually have millisecondsPerCall to play. If we haven't, there has been a
    ...
    /* After this, regardless of what has happened, deliver millisecondsPerCall of future data */
    inst->curPosition += inst->timestampsPerCall;
    *pw16_len = inst->timestampsPerCall;
