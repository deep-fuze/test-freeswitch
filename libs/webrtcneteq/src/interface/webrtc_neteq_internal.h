...
typedef struct {
  ...
  /*
   * Number of packets (by RTP sequence number) that never made to decoder
   */
   ...
   */
  int total_insert_errors;
  int total_extract_errors;
  int pkts_in_buffer; //At present
  int max_pkts_in_buffer; //At present
 ...
 * If clear is set, then it will clear the stats.
 * e.g. per RecOut() call.
 */
void WebRtcNetEQ_GetProcessingActivity(void* inst,
                                       WebRtcNetEQ_ProcessingActivity* stat, int clear);

/*
 * Get the current size of WebRtcNeteq buffer size
 */
...
/*
 * Get the Maximum number of packets we can store for a given inst.
 */
