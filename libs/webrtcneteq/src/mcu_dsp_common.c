...
int WebRtcNetEQ_DSP2MCUinterrupt(MainInst_t *inst, int16_t *pw16_shared_mem)
{
    ...
    return WebRtcNetEQ_SignalMcu(&inst->MCUinst, &inst->last_seq_no, &inst->consecutive_lost, &inst->total_decoded);
}
...
int WebRtcNetEQ_SetPacketSize(MainInst_t *inst, uint16_t *packet_ms)
{
    inst->DSPinst.millisecondsPerCall = packet_ms;
    inst->MCUinst.millisecondsPerCall = packet_ms;
