                                         ...
                                         int* per_packet_overhead_bytes)
{
    ...
    *sizeinbytes = (*sizeinbytes) * multiplier;
    ...
    NetEqMainInst->max_buf_len = *MaxNoOfPackets;
    return 0;
}
...
int WebRtcNetEQ_Init(void *inst, uint16_t fs, uint16_t packet_ms)
{
    int ok = 0;
    ...
    ok = WebRtcNetEQ_SetPacketSize(NetEqMainInst, packet_ms);
    RETURN_ON_ERROR(ok, NetEqMainInst);
                      ...
                      uint32_t uw32_timeRec)
{
    ...
    if (NetEqMainInst->MCUinst.main_inst != NetEqMainInst)
    {
        /* Instance is corrupt */
        NetEqMainInst->ErrorCode = CORRUPT_INSTANCE;
	NetEqMainInst->DSPinst.activity_stats.total_insert_errors++;
        return (-2);
    }
    ...
    ok = WebRtcNetEQ_RecInInternal(&NetEqMainInst->MCUinst, &RTPpacket, uw32_timeRec, &flushed);
    if (ok != 0)
    {
        NetEqMainInst->ErrorCode = -ok;
	NetEqMainInst->DSPinst.activity_stats.total_insert_errors++;
                       ...
                       uint16_t *consecutive_lost, uint32_t *total_decoded, uint8_t suppress_lost)
{
    ...
    if (NetEqMainInst == NULL) {
	*pw16_len = 0;
	NetEqMainInst->DSPinst.activity_stats.total_extract_errors++;
	return (-1);
    }

    if (!NetEqMainInst->seenRecIn) {
	*pw16_len = 0;
	NetEqMainInst->DSPinst.activity_stats.total_extract_errors++;
	return (-2);
    }
    ...
    if (suppress_lost == 0) {
        *consecutive_lost = NetEqMainInst->consecutive_lost;
        NetEqMainInst->DSPinst.activity_stats.total_lost_count += *consecutive_lost;
    } else {
        *consecutive_lost = 0;
    }
    *total_decoded = NetEqMainInst->total_decoded;
...
void WebRtcNetEQ_GetProcessingActivity(void *inst,
  ...
  stats->preemptive_expand_normal_samples =
      NetEqMainInst->DSPinst.activity_stats.preemptive_expand_normal_samples; //Too low
  ...
  stats->merge_expand_normal_samples =
      NetEqMainInst->DSPinst.activity_stats.merge_expand_normal_samples; //First packet after expand ends

  stats->total_lost_count = NetEqMainInst->DSPinst.activity_stats.total_lost_count;
  stats->total_insert_errors = NetEqMainInst->DSPinst.activity_stats.total_insert_errors;
  stats->total_extract_errors = NetEqMainInst->DSPinst.activity_stats.total_extract_errors;
  stats->pkts_in_buffer = NetEqMainInst->MCUinst.PacketBuffer_inst.numPacketsInBuffer;
  stats->max_pkts_in_buffer = NetEqMainInst->MCUinst.PacketBuffer_inst.maxNumPacketsInBuffer;
...
void WebRtcNetEQ_GetJitterBufferSize(void* inst, WebRtcNetEQ_ProcessingActivity *stats) {
    MainInst_t *NetEqMainInst = (MainInst_t*) inst;
    stats->pkts_in_buffer = NetEqMainInst->MCUinst.PacketBuffer_inst.numPacketsInBuffer;
}
...
uint16_t WebRtcNetEQ_MaxBufLen(const void* inst) {
    const MainInst_t *NetEqMainInst = (const MainInst_t*) inst;
    if (NetEqMainInst)
        return NetEqMainInst->max_buf_len;
