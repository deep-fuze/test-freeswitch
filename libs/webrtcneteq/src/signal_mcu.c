 ...
 */
int WebRtcNetEQ_SignalMcu(MCUInst_t *inst, uint16_t *last_seq_no, uint16_t *consecutive_lost, uint32_t *total_decoded)
{
    ...
    *consecutive_lost = 0;
    ...
    if (inst->new_codec != 1)
    {
        if (WebRtcNetEQ_DbIsMDCodec((enum WebRtcNetEQDecoder) inst->current_Codec))
        {
                    ...
                    (enum WebRtcNetEQDecoder) inst->current_Codec) == payloadType))
            {
                ...
                (*total_decoded)++;	
		        if (*last_seq_no && temp_pkt.seqNumber > (*last_seq_no + 1)){
			        *consecutive_lost += temp_pkt.seqNumber - (*last_seq_no + 1);
		        }
		        *last_seq_no = temp_pkt.seqNumber;
    ...
    uw16_instr = WebRtcNetEQ_BufstatsDecision(&inst->BufferStat_inst,
        ...
        lastModeBGNonly, playDtmf, inst->millisecondsPerCall / 10);

    inst->consecutive_non_accelerates++;
    ...
    if ((inst->new_codec) || (uw16_instr == BUFSTAT_REINIT))
    {
        ...
        *blockPtr = blockLen * 2;
        ...
        inst->PacketBuffer_inst.packSizeSamples =
            WebRtcNetEQ_UpdatePackSizeSamples(inst, i_bufferpos, payloadType,
				30 * (inst->timestampsPerCall / inst->millisecondsPerCall));
    ...
    {
	int ts_mult = inst->millisecondsPerCall / 10;
        /* In order to do a Accelerate we need at least 30ms of data */
        if (dspInfo.samplesLeft >= (3 * 80 * fs_mult * ts_mult))
        {
	    ...
	    inst->consecutive_non_accelerates = 0;
            return 0;
        }
        else if ((dspInfo.samplesLeft >= (1 * 80 * fs_mult * ts_mult))
            && (inst->PacketBuffer_inst.packSizeSamples >= (240 * fs_mult * ts_mult)))
        {
            ...
            && (inst->PacketBuffer_inst.packSizeSamples >= (240 * fs_mult * ts_mult)))
        {
            ...
            wantedNoOfTimeStamps = 10 * (inst->timestampsPerCall / inst->millisecondsPerCall) * ts_mult;
	    inst->consecutive_non_accelerates = 0;
        }
        else if (dspInfo.samplesLeft >= (2 * 80 * fs_mult * ts_mult) && 
			inst->consecutive_non_accelerates > (200 / inst->millisecondsPerCall)) 
			/*Don't accelerate too often. at most once in 200ms*/
        {
            /* We need to decode another 10ms in order to do an Accelerate */
            wantedNoOfTimeStamps = 10 * (inst->timestampsPerCall / inst->millisecondsPerCall) * ts_mult;
	    inst->consecutive_non_accelerates = 0;
        }
        else
        {
             ...
             */
            wantedNoOfTimeStamps = 20 * (inst->timestampsPerCall / inst->millisecondsPerCall) * ts_mult;
            uw16_instr = BUFSTATS_DO_NORMAL;
        }
    }
        ...
        || (uw16_instr == BUFSTATS_DO_PREEMPTIVE_EXPAND))
    {
        ...
        if ((totalTS + dspInfo.samplesLeft) < WEBRTC_SPL_MUL(30, (inst->timestampsPerCall / inst->millisecondsPerCall))
