...
static void
add_data(struct evhttp_request *req, const char *file)
{
	int8_t buf[1024];
	FILE *fp = fopen(file, "rb");
	int rlen, total_len = 0;
	
	while ((rlen = fread(buf, 1, sizeof(buf), fp)) == sizeof(buf)) {
		evbuffer_add(evhttp_request_get_output_buffer(req), buf, rlen);
		total_len += rlen;
	}
	
	if (rlen && rlen < sizeof(buf)) {	
		evbuffer_add(evhttp_request_get_output_buffer(req), buf, rlen);
		total_len += rlen;
	}
...
main(int argc, char **argv)
{
	...
	const char *url, *scheme, *host, *path, *query, *media_file = NULL;
	char uri[256];
	int port;
	...
	evhttp_add_header(output_headers, "Content-Type", "audio/x-flac; rate=16000");
	media_file = "conf-alone.flac";
	if (media_file)
		add_data(req, media_file);
