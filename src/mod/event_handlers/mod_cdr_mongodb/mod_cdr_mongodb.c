...
#define bson_append_string_check(cdr, key, val) \
	{ if (switch_core_log_check(session, key)) bson_append_string(cdr, key, val); }

#define bson_append_long_check(cdr, key, val) \
	{ if (switch_core_log_check(session, key)) bson_append_long(cdr, key, val); }
/*
...
*/
static void set_bson_profile_data(switch_core_session_t *session, bson *b, switch_caller_profile_t *caller_profile)
{
	bson_append_string_check(b, "username", caller_profile->username);
	bson_append_string_check(b, "dialplan", caller_profile->dialplan);
	bson_append_string_check(b, "caller_id_name", caller_profile->caller_id_name);
	bson_append_string_check(b, "ani", caller_profile->ani);
	bson_append_string_check(b, "aniii", caller_profile->aniii);
	bson_append_string_check(b, "caller_id_number", caller_profile->caller_id_number);
	bson_append_string_check(b, "network_addr", caller_profile->network_addr);
	bson_append_string_check(b, "rdnis", caller_profile->rdnis);
	bson_append_string_check(b, "destination_number", caller_profile->destination_number);
	bson_append_string_check(b, "uuid", caller_profile->uuid);
	bson_append_string_check(b, "source", caller_profile->source);
	bson_append_string_check(b, "context", caller_profile->context);
	bson_append_string_check(b, "chan_name", caller_profile->chan_name);
}
...
static void report_adhoc(switch_core_session_t *session)
{
	bson cdr;
	...
	if (!session) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "NULL session. Can't report periodic stats.\n");
		return;
	}
	...
	bson_append_start_object(&cdr, "channel_periodic_data");

	bson_append_date(&cdr, "created_on", (bson_date_t) (switch_micro_time_now() / 1000));
	bson_append_string(&cdr, "version", STATS_VERSION);
	cur_idx = switch_get_first_running_stat(channel, &key, &value);
	while(cur_idx >= 0 && key && value) {
		bson_append_string(&cdr, key, value);
...
static switch_status_t my_on_reporting(switch_core_session_t *session)
{
	...
	bson_append_string_check(&cdr, "state", switch_channel_state_name(switch_channel_get_state(channel)));
	bson_append_string_check(&cdr, "direction", switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_OUTBOUND ? "outbound" : "inbound");
	bson_append_int(&cdr, "state_number", switch_channel_get_state(channel));

	if ((tmp = switch_channel_get_flag_string(channel))) {
		bson_append_string_check(&cdr, "flags", tmp);
		free(tmp);
	}

	if ((tmp = switch_channel_get_cap_string(channel))) {
		bson_append_string_check(&cdr, "caps", tmp);
		free(tmp);
	}
	...
	if ((hi = switch_channel_variable_first(channel))) {
		for (; hi; hi = hi->next) {
			if (!zstr(hi->name) && !zstr(hi->value)) {
				bson_append_string_check(&cdr, hi->name, hi->value);
			}
		}
	...
	if ((app_log = switch_core_session_get_app_log(session))) {
		...
		for (ap = app_log; ap; ap = ap->next) {
			bson_append_string_check(&cdr, "app_name", ap->app);
			bson_append_string_check(&cdr, "app_data", switch_str_nil(ap->arg));
			bson_append_long_check(&cdr, "app_stamp", ap->stamp);
		}
	}
	...
	if (hold_record) {
		...
		for (hr = hold_record; hr ; hr = hr->next) {
			bson_append_long_check(&cdr, "on", hr->on);
			bson_append_long_check(&cdr, "off", hr->off);
			if (hr->uuid) {
				bson_append_string_check(&cdr, "bridged_to", hr->uuid);
			}
			idx++;
		}
	}
	...
	while (caller_profile) {
		...
		if (!zstr(caller_profile->dialplan)) {
			bson_append_string_check(&cdr, "dialplan", caller_profile->dialplan);
		}

		if (!zstr(caller_profile->profile_index)) {
			bson_append_string_check(&cdr, "profile_index", caller_profile->profile_index);
		}
		...
		if (caller_profile->caller_extension) {
			...
			bson_append_string_check(&cdr, "name", switch_str_nil(caller_profile->caller_extension->extension_name));
			bson_append_string_check(&cdr, "number", switch_str_nil(caller_profile->caller_extension->extension_number));

			if (caller_profile->caller_extension->current_application) {
				bson_append_string_check(&cdr, "current_app", caller_profile->caller_extension->current_application->application_name);
			}
			...
			for (ap = caller_profile->caller_extension->applications; ap; ap = ap->next) {
				...
				bson_append_string_check(&cdr, "app_name", ap->application_name);
				bson_append_string_check(&cdr, "app_data", switch_str_nil(ap->application_data));
			}
			...
			if (caller_profile->caller_extension->children) {
				...
				for (cp = caller_profile->caller_extension->children; cp; cp = cp->next) {
					...
					bson_append_string_check(&cdr, "name", cp->caller_extension->extension_name);
					bson_append_string_check(&cdr, "number", cp->caller_extension->extension_number);
					bson_append_string_check(&cdr, "dialplan", cp->dialplan);
					if (cp->caller_extension->current_application) {
						bson_append_string_check(&cdr, "current_app", cp->caller_extension->current_application->application_name);
					}
		...
		if (caller_profile->times) {
			...
			bson_append_long_check(&cdr, "created_time", caller_profile->times->created);
			bson_append_long_check(&cdr, "profile_created_time", caller_profile->times->profile_created);
			bson_append_long_check(&cdr, "progress_time", caller_profile->times->progress);
			bson_append_long_check(&cdr, "progress_media_time", caller_profile->times->progress_media);
			bson_append_long_check(&cdr, "answered_time", caller_profile->times->answered);
			bson_append_long_check(&cdr, "bridged_time", caller_profile->times->bridged);
			bson_append_long_check(&cdr, "last_hold_time", caller_profile->times->last_hold);
			bson_append_long_check(&cdr, "hold_accum_time", caller_profile->times->hold_accum);
			bson_append_long_check(&cdr, "hangup_time", caller_profile->times->hungup);
			bson_append_long_check(&cdr, "resurrect_time", caller_profile->times->resurrected);
			bson_append_long_check(&cdr, "transfer_time", caller_profile->times->transferred);
		}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_cdr_mongodb_load)
{
	...
	switch_core_add_periodic_log_handler(report_adhoc);
