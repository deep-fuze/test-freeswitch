...
static void skip_leading_space(char **s)
{
    char *p = *s;
    while (p && *p)
    {
      ...
      filter_end:
        ...
        if (events) {
            ...
            for (cur = edup; cur; count++) {
                ...
                if (custom) {
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Insert custom subscription %s\n", cur);
...
static void *SWITCH_THREAD_FUNC api_exec(switch_thread_t *thread, void *obj)
{
    ...
    if (acs->bg) {
        ...
        if (acs->api_request_id)
        {
            switch_snprintf(
                buf, sizeof(buf), 
                "Content-Type: api/response\nContent-Length: %" SWITCH_SSIZE_T_FMT "\nAPI-Request-ID: %s\n\n", 
                rlen, acs->api_request_id
            );
        }
        else
        {
...
static switch_status_t parse_command(listener_t *listener, switch_event_t **event, char *reply, uint32_t reply_len)
{
    ...
    *reply = '\0';
    ...
    if (!strncasecmp(cmd, "sendevent", 9)) {
        ...
        char *request_id = cmd + 4;
        skip_leading_space(&request_id);
        if (!strncasecmp(request_id, "API-Request-ID", 14))
        {
            ...
            if (request_id && *request_id == '=')
            {
                ...
                if (request_id)
                {
                    ...
                    switch_log_printf(
                        SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "API-Request-ID=%s\n", request_id
                    );
                }
            }
        }
...
static void *SWITCH_THREAD_FUNC listener_run(switch_thread_t *thread, void *obj)
{
    ...
    if (zstr(listener->remote_ip)) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Connection Open\n");
    } else {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Connection Open from %s:%d\n", listener->remote_ip,
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Connection Closed\n");
