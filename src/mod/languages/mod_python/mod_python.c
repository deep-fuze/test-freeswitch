...
static switch_mutex_t *fm_script_lock = NULL;
static PyObject *fm_module = NULL;
...
#if 0
	if (!strcmp(script, "fuzemeeting")) {
		switch_mutex_lock(fm_script_lock);

		if (fm_module == NULL) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Import python script (opt) %s\n", script);
			fm_module = PyImport_ImportModule((char *) script);
			if (!fm_module) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Error importing fm_module\n");
...
#if 1
		fm_module = PyImport_ReloadModule(fm_module);
		if (!fm_module) {
		...
		function = PyObject_GetAttrString(fm_module, (char *) funcname);
		if (!function) {
		...
		if (session) {
			sp = mod_python_conjure_session(fm_module, session);
		}
		...
		// invoke the handler                                                                                                                   
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Call python script (opt) %s\n", script);
		result = PyEval_CallObjectWithKeywords(function, arg, (PyObject *) NULL);
		switch_mutex_unlock(fm_script_lock);
		...
		// invoke the handler 
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Call python script %s\n", script);
		result = PyEval_CallObjectWithKeywords(function, arg, (PyObject *) NULL);
		Py_DECREF(function);
	}

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Finished calling python script time:%" PRId64 "\n", now);
...
static void set_max_recursion_depth(void)
{
	...
	if (setrecursion_result) {
		Py_DECREF(setrecursion_result);
	}
	if (recLimit) {
		Py_DECREF(recLimit);
	}
    if (setRecursionLimit) {
		Py_DECREF(setRecursionLimit);
    }
    if (sysModule) {
		Py_DECREF(sysModule);
    }
...
SWITCH_MODULE_LOAD_FUNCTION(mod_python_load)
{
	...
	switch_mutex_init(&fm_script_lock, SWITCH_MUTEX_NESTED, pool);
