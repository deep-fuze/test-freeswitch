...
struct mod_sofia_globals {
	...
	/*Fuze Transport Base*/
	/*Currently we use only for data plane. so single base is enough.
	* More over Fuze transport supports only one base
	*/
	switch_bool_t use_fuze_transport;
	void *tbase;
...
void sofia_glue_set_periodic_stats(switch_core_session_t *session, switch_bool_t event_trigger);
switch_status_t sofia_glue_stats_ioctl(switch_core_session_t *session, int cmd, void *arg);
