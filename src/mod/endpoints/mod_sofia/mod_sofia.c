...
static jb_t *sofia_get_jb(switch_core_session_t *session, switch_media_type_t type)
{
    private_object_t *tech_pvt = (private_object_t *) switch_core_session_get_private(session);
...
static int hangup_cause_to_sip(switch_call_cause_t cause)
{
    ...
    case SWITCH_CAUSE_EXCHANGE_ROUTING_ERROR:
        return 483;
    case SWITCH_CAUSE_CONFERENCE_INACTIVE:
        return 581;
    default:
        return 480;
    }
...
switch_status_t sofia_on_hangup(switch_core_session_t *session)
{
    ...
    if (sofia_test_pflag(tech_pvt->profile, PFLAG_DESTROY)) {
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Sending BYE to %s reason %s\n", switch_channel_get_name(channel), reason);
...
#else
    if (!(switch_core_media_ready(tech_pvt->session, SWITCH_MEDIA_TYPE_AUDIO) && !switch_channel_test_flag(channel, CF_REQ_MEDIA))) {
        switch_ivr_parse_all_messages(tech_pvt->session);
        return SWITCH_STATUS_FALSE;
    }
#endif
...
SWITCH_MODULE_LOAD_FUNCTION(mod_sofia_load)
{
    ...
    if (mod_sofia_globals.use_fuze_transport == SWITCH_TRUE) {
        fuze_transport_register_trace_cb(switch_log_print_msg);
        fuze_transport_init(1);

        if ((mod_sofia_globals.tbase = fuze_transport_create_transport_base(PT_HIGH)) == NULL) {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't initialize transport.\n");
            return SWITCH_STATUS_GENERR;
        }
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Sofia: Using Fuze Transport.\n");
    } else {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Sofia: Using Legacy Transport.\n");
    }
    ...
    *module_interface = switch_loadable_module_create_module_interface(pool, modname);
    ...
    sofia_endpoint_interface->stats_ioctl = sofia_glue_stats_ioctl;
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_sofia_shutdown)
{
    ...
    fuze_transport_destroy_transport_base(mod_sofia_globals.tbase);
