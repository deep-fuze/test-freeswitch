									 ...
									 const char *host_var, const char *port_var, const char *uri_var, const char *params_var)
{
	...
	if (host) {
		...
		if (strstr(host_var, "sip_req") && strlen(host) >= 4) { 
                	char location[4];
                   	strncpy(location, host, 3);
                       	location[3] = '\0';
                       	switch_channel_set_variable(channel, "location", location);
			if (switch_channel_update_running_stats(channel, "location", location) < 0) 
                       		switch_channel_set_running_stats(channel, "location", location);
               }
	}
						 ...
						 switch_core_session_t *session)
{
	...
	if (sip) {
		if (sip->sip_from && sip->sip_from->a_url) {
			...
			if ((vval = switch_channel_get_variable(channel, "sip_from_uri"))) {
                		if (switch_channel_update_running_stats(channel, "sip_from_uri", vval) < 0) {
                        		switch_channel_set_running_stats(channel, "sip_from_uri", vval);
                		}
			}
		}
		...
		if (sip->sip_to && sip->sip_to->a_url) {
			...
			if ((vval = switch_channel_get_variable(channel, "sip_to_uri"))) {
                		if (switch_channel_update_running_stats(channel, "sip_to_uri", vval) < 0) {
                        		switch_channel_set_running_stats(channel, "sip_to_uri", vval);
                		}
        		}
		}
...
static void set_call_id(private_object_t *tech_pvt, sip_t const *sip)
{
	...
	if (tech_pvt->channel && tech_pvt->call_id && 
		switch_channel_update_running_stats(tech_pvt->channel, "sip_call_id", tech_pvt->call_id) < 0) {	

		switch_channel_set_running_stats(tech_pvt->channel, "sip_call_id", tech_pvt->call_id);	
	}
}
...
switch_status_t config_sofia(sofia_config_t reload, char *profile_name)
{
	...
	if ((settings = switch_xml_child(cfg, "global_settings"))) {
		for (param = switch_xml_child(settings, "param"); param; param = param->next) {
		    ...
		    } else if (!strcasecmp(var, "use-new-transport") && !strcasecmp(val, "true")) {
				mod_sofia_globals.use_fuze_transport = SWITCH_TRUE;
		    }
		}
	}
									  ...
									  tagi_t tags[])
{
	...
	if (sip && session) {
		...
		if (sip->sip_payload && sip->sip_payload->pl_data) {
			...
			switch_core_media_set_extmap_string(session, sip->sip_payload->pl_data);
			switch_core_media_set_session_name(session, sip->sip_payload->pl_data);
