													...
													switch_originate_flag_t flags, switch_call_cause_t *cancel_cause)
{
    ...
    if (!(tech_pvt->rtp_session = switch_rtp_new(NULL, local_addr, local_port, remote_addr, remote_port, tech_pvt->agreed_pt,
    ...
    switch_rtp_apply_timestamp_multiplier(tech_pvt->rtp_session, tech_pvt->read_codec.implementation->timestamp_multiplier);
...
static switch_status_t channel_write_frame(switch_core_session_t *session, switch_frame_t *frame, switch_io_flag_t flags, int stream_id)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "channel_write_frame mode=%d\n", tech_pvt->mode);
			...
			if (switch_rtp_ready(tech_pvt->rtp_session)) {
				...
				if (msg->string_arg) {
					...
					if (!strcasecmp(msg->string_arg, "pause")) {
						...
						goto end;
                                                ...
                                                if (s && !strcmp(s, "true")) {
                                                        switch_rtp_set_webrtc_neteq(tech_pvt->rtp_session, SWITCH_TRUE);
							switch_channel_set_variable(tech_pvt->channel, "use_webrtc_neteq", "true");
							switch_channel_set_variable(tech_pvt->channel, "jitterbuffer_impl", "neteq");
                                                        break;
                                                }
                                        }
				...
				if (qlen) {
					...
					if (switch_rtp_activate_jitter_buffer(tech_pvt->rtp_session, qlen, maxqlen,
						...
						switch_channel_set_variable(tech_pvt->channel, "jitterbuffer_impl", "freeswitch");
