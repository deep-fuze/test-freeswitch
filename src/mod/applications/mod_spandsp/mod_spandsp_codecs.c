...
static void* switch_g722_decoder(switch_codec_t *codec)
{
        struct g722_context *context;
...
switch_status_t mod_spandsp_codecs_load(switch_loadable_module_interface_t **module_interface, switch_memory_pool_t *pool)
{
	...
	for (count = 6; count > 0; count--) {
		...
		switch_core_codec_implementation_set_decoder(codec_interface, switch_g722_decoder);
	}
	...
	for (count = 12; count > 0; count--) {
		...
		switch_core_codec_implementation_set_decoder(codec_interface, switch_g711u_decoder);
	}
	...
	for (count = 12; count > 0; count--) {
		...
		switch_core_codec_implementation_set_decoder(codec_interface, switch_g711a_decoder);
	}
#endif
