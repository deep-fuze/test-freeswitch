...
{
    char dumpstr[DUMP_BUFFER_SIZE];
    size_t i;
    size_t c;
    unsigned int width = 80;

    switch_snprintf(dumpstr, DUMP_BUFFER_SIZE, "%s %s, %10.10ld bytes (0x%8.8lx)\n", dumpstr, text, (long)size, (long)size);

    for(i = 0; i<size; i += width) {

        switch_snprintf(dumpstr, DUMP_BUFFER_SIZE, "%s%4.4lx: ", dumpstr, (long)i);
        ...
        for(c = 0; (c < width) && (i + c < size); c++) {
            ...
            switch_snprintf(dumpstr, DUMP_BUFFER_SIZE, "%s%c", dumpstr,
        ...
        switch_snprintf(dumpstr, DUMP_BUFFER_SIZE, "%s\n", dumpstr);
    }
             ...
             void *userp)
{
    ...
    case CURLINFO_TEXT:
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "== Info: %s", data);
        /* FALLTHROUGH */
    ...
    if (!strcasecmp(method, "head")) {
        ...
        if (content_type) {
            ...
            if (options && options->authorization) {
                char *auth = switch_mprintf("Authorization: Bearer %s", options->authorization);
                headers = switch_curl_slist_append(headers, auth);
                switch_safe_free(auth);
            }
...
SWITCH_STANDARD_APP(curl_app_function)
{
    ...
    if ((argc = switch_separate_string(mydata, ' ', argv, (sizeof(argv) / sizeof(argv[0]))))) {
        ...
        for (i = 1; i < argc; i++) {
            ...
            } else if (!strcasecmp("authorization-bearer", argv[i])) {
                if (++i < argc) {
                    authorization = switch_core_strdup(pool, argv[i]);
                }
            }
        }
    }
