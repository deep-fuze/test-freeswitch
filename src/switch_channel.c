...
static struct switch_cause_table CAUSE_CHART[] = {
    ...
    {"CONFERENCE_INACTIVE", SWITCH_CAUSE_CONFERENCE_INACTIVE},
    {NULL, 0}
};
...
struct switch_channel {
    ...
    char *device_id;
    running_stat_t stats[MAX_RUNNING_STATS];
...
static void switch_channel_fire_call_event(switch_channel_t *channel);
static switch_status_t switch_channel_check_return_code(switch_status_t res, const char *file, const char *func, int line);

SWITCH_DECLARE(switch_status_t) switch_channel_check_return_code(switch_status_t res, const char *file, const char *func, int line) 
{
    if (res != SWITCH_STATUS_SUCCESS)
    {
        switch_log_printf(SWITCH_CHANNEL_LOG, 
            SWITCH_LOG_ERROR, "Function call failed. Error code: %d, file: %s, function: %s, line: %d.\n", 
            res, file, func, line);
    }
    return res;
}

#define CHCK_ERR(func_call_res) switch_channel_check_return_code(func_call_res, __FILE__, (const char * )__SWITCH_FUNC__, __LINE__)
...
SWITCH_DECLARE(void) switch_channel_uninit(switch_channel_t *channel)
{
    ...
    switch_event_destroy(&channel->call_event);
    switch_mutex_unlock(channel->profile_mutex);
}
...
SWITCH_DECLARE(void) switch_channel_change_thread(switch_channel_t *channel) {
    switch_core_session_change_thread(channel->session);
}
...
SWITCH_DECLARE(switch_channel_state_t) switch_channel_perform_hangup(switch_channel_t *channel,
                                                                     const char *file, const char *func, int line, switch_call_cause_t hangup_cause, const char *cause_str)
{
    int ok = 0;
    ...
    if (channel->state < CS_HANGUP) {
        ...
        if (cause_str) 
            switch_channel_set_variable(channel, "app_hangup_cause", cause_str);    
    }
...
static void switch_channel_copy_stats(switch_channel_t *channel, int idx, 
                        const char* key, const char* value)
{
    ...
    len = MIN(sizeof(channel->stats[idx].key) - 1, strlen(key));
    memcpy(channel->stats[idx].key, key, len);
    channel->stats[idx].key[len] = 0;

    len = MIN(sizeof(channel->stats[idx].value) - 1, strlen(value));
    memcpy(channel->stats[idx].value, value, len);  
    channel->stats[idx].value[len] = 0;
    
    channel->stats[idx].mask = 0;
}

SWITCH_DECLARE(int) switch_channel_set_running_stats(switch_channel_t *channel, const char* key, 
                                const char* value)
{
    int idx;
    ...
    idx = channel->next_stat_index++;

    switch_channel_copy_stats(channel, idx, key, value);
    if (channel->next_stat_index == ARRAY_NUM_ELEM(channel->stats))
        channel->next_stat_index = 0;
    
    return idx; 
}

SWITCH_DECLARE(int) switch_channel_update_running_stats_by_idx(switch_channel_t *channel, int idx,
                                const char* key, const char* value)
{
    if (!channel || idx < 0 || idx >= ARRAY_NUM_ELEM(channel->stats))
        return -1;

    if (strncmp(channel->stats[idx].key, key, sizeof(channel->stats[idx].key) - 1))
        return -1;
...
SWITCH_DECLARE(int) switch_channel_update_running_stats(switch_channel_t *channel, 
                             const char* key, const char* value)
{
    int idx;
    int len;
    ...
    len = ARRAY_NUM_ELEM(channel->stats);
...
SWITCH_DECLARE(void) switch_mask_running_stat_by_idx(switch_channel_t *channel, int idx)
{
    if (idx < 0 || idx >= ARRAY_NUM_ELEM(channel->stats))
        return;

    channel->stats[idx].mask = 1;
}

SWITCH_DECLARE(void) switch_mask_running_stat(switch_channel_t *channel, const char* key)
{
    int idx;
    int len = ARRAY_NUM_ELEM(channel->stats);
...
SWITCH_DECLARE(int) switch_get_first_running_stat(switch_channel_t *channel, const char** key,  const char** value)
{
    int idx;
    int len;
    ...
    for (idx = 0; idx < len; idx++) {
        if (strlen(channel->stats[idx].key) && !channel->stats[idx].mask) {
            *key = channel->stats[idx].key;
            *value = channel->stats[idx].value;
            return idx;
        }
    }
...
SWITCH_DECLARE(int) switch_get_next_running_stat(switch_channel_t *channel, int cur_idx, const char** key,  const char** value)
{
    int idx;
    int len;

    if (!channel || cur_idx < 0)
        return -1;

    len = ARRAY_NUM_ELEM(channel->stats);

    for (idx = cur_idx + 1; idx < len; idx++) {
        if (strlen(channel->stats[idx].key) && !channel->stats[idx].mask) {
                        *key = channel->stats[idx].key;
                        *value = channel->stats[idx].value;
                        return idx;
...
SWITCH_DECLARE(void) switch_channel_fire_call_event(switch_channel_t *channel)
{
    if (CHCK_ERR(switch_event_add_header(channel->call_event, SWITCH_STACK_BOTTOM, "Number-Events", "%d", channel->call_events_count)) == SWITCH_STATUS_SUCCESS) {
        CHCK_ERR(switch_event_fire(&channel->call_event));
    } else {
        switch_event_destroy(&channel->call_event); 
    }
    channel->call_event = NULL;
}

SWITCH_DECLARE(void) switch_channel_publish_call_events(switch_channel_t *channel, switch_bool_t force, switch_bool_t flush)
{
    int cur_idx;
    const char *key, *value;
    char buff[256] = "";
    switch_time_t now;
    switch_time_t max_time_between_pushes;
    ...
    if (!channel->call_event) {
        if (CHCK_ERR(switch_event_create_subclass(&channel->call_event, SWITCH_EVENT_CUSTOM, SWITCH_CHANNEL_CALL_EVENT)) == SWITCH_STATUS_SUCCESS) {
            // Action field is already used inside FN when consuming conference events
            // adding it here so that we don't have to change existing framework
            CHCK_ERR(switch_event_add_header_string(channel->call_event, SWITCH_STACK_BOTTOM, "Action", SWITCH_CHANNEL_CALL_EVENT));
            channel->call_events_count = 0;
        } else {
            return;
        }       
    }

    cur_idx = switch_get_first_running_stat(channel, &key, &value);
    while (cur_idx >= 0 && key && value) {
        // The values inside the event will be in the form:
        // 0.rtp_max...
        // 0.rtp_event_name...
        // ...
        // 1.rtp_max...
        switch_snprintf(buff, sizeof(buff), "%d.%s", channel->call_events_count, key);
        if (CHCK_ERR(switch_event_add_header_string(channel->call_event, SWITCH_STACK_BOTTOM, buff, value)) != SWITCH_STATUS_SUCCESS) {
            force = SWITCH_TRUE; // my assumption is that if we can't add anymore data, let's send the event and start the new one
            break;
        }
    ...
    if (switch_rtp_get_muted(channel)) {
        max_time_between_pushes = MAX_INTER_STATS_PUBLISH_TIME_MUTED;
    } else {
        max_time_between_pushes = MAX_INTER_STATS_PUBLISH_TIME;
    }

    if ((now - channel->last_event_push) > max_time_between_pushes || force || channel->call_events_count == MAX_NUMBER_OF_EVENTS) {
        switch_time_t delta = now - channel->last_event_push;

        switch_log_printf(SWITCH_CHANNEL_CHANNEL_LOG(channel), SWITCH_LOG_DEBUG,
                          "pushing stats because force=%d count=%d (max=%d) delta=%" PRId64 "s (max=%" PRId64 "s)\n",
                          force, channel->call_events_count, MAX_NUMBER_OF_EVENTS, (delta)/(1000*1000),
						  max_time_between_pushes/(1000*1000));
