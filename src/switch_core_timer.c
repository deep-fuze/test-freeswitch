													   ...
													   switch_memory_pool_t *pool)
{
	...
	if (pool) {
		...
		if ((status = switch_core_new_memory_pool(&timer->memory_pool)) != SWITCH_STATUS_SUCCESS) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "switch_core_new_memory_pool not successful!\n");
...
{
    if (!timer->timer_interface || !timer->timer_interface->timer_now_us) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Timer is not properly configured.\n");
        *now_us = 0;
		return SWITCH_STATUS_GENERR;
    }

    return timer->timer_interface->timer_now_us(now_us);
}
