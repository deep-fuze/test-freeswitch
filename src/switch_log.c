...
SWITCH_DECLARE(void) switch_log_print(int16_t level, const char * fmt, ...)
{
        va_list ap;
        va_start(ap, fmt);
        switch_log_vprintf(SWITCH_CHANNEL_LOG, level, fmt, ap);
        va_end(ap);
}

SWITCH_DECLARE(void) switch_log_print_msg(int16_t level, const char * msg)
{
    switch_log_printf(SWITCH_CHANNEL_LOG, level, "%s\n", msg);
}
