...
struct switch_core_session {
        ...
        size_t last_rtp_read_pkt_count;
        int monitor_index;
        switch_media_handle_t *media_handle;
        uint32_t decoder_errors;

        /* fuze */
        char email[SWITCH_CORE_EMAIL_LEN];
        char phone[SWITCH_CORE_PHONE_LEN];
        uint8_t suppress_lost;
        uint32_t audio_level_low_count;
        switch_bool_t in_cn_period;
        switch_time_t period_start;
        int pkt_errors;
        switch_bool_t cantspeak;


        uint32_t insert;
        uint32_t read;
        uint32_t skip_insert;
        uint32_t skip_read;
        uint32_t errors;
        uint32_t hot_read;
        uint32_t empty;
...
struct switch_runtime {
        ...
        switch_periodic_log_t periodic_log;
