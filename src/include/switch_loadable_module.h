...
static inline void switch_core_codec_add_implementation_w_ctl(switch_memory_pool_t *pool, switch_codec_interface_t *codec_interface,
                                                              /*! enumeration defining the type of the codec */
                                                              const switch_codec_type_t codec_type,
                                                              /*! the IANA code number */
                                                              switch_payload_t ianacode,
...
static inline void switch_core_codec_implementation_set_decoder
			(switch_codec_interface_t *codec_interface, switch_core_codec_decoder_t decoder)
{
      	switch_codec_implementation_t *impl;
	if (!codec_interface)
		return;

	impl = codec_interface->implementations;
       	while(impl) {
...
static inline void switch_core_codec_set_timestamp_multiplier(switch_codec_interface_t *codec_interface, 
								uint8_t multiplier)
{
	if (!codec_interface || !codec_interface->implementations)
		return;

	codec_interface->implementations->timestamp_multiplier = multiplier;
}
