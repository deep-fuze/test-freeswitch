...
typedef jb_t *(*switch_io_get_jb_t) (switch_core_session_t *, switch_media_type_t);
...
struct switch_endpoint_interface {
	...
	switch_stats_ioctl_t stats_ioctl;
};
...
struct switch_timer_interface {
    ...
    /* function to return softtimer. May not be as accurate as hw time*/
    switch_status_t (*timer_now_us) (switch_time_t *now_us);
...
struct switch_codec_implementation {
	...
	/*! actual samples transferred per second for those who are not moron g722 RFC writers */
	uint32_t actual_samples_per_second;
	/*! Multiplier for RTP timestamp. samples_per_second are multiplied 
	    by this number for each packet. For most codecs, it is 1.*/
	...
	switch_core_codec_decoder_t decoder;
};
