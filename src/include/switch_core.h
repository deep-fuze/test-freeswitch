...
#define FUZE_TIMER_MISS_INTERVAL 60000
...
/*
 * Register console API for session related commands.
 */
...
SWITCH_DECLARE(switch_status_t) switch_core_session_read_frame(_In_ switch_core_session_t *session, switch_frame_t **frame, switch_io_flag_t flags,
                                                               int stream_id);
SWITCH_DECLARE(switch_status_t) switch_core_session_read_frame_w_time(_In_ switch_core_session_t *session, switch_frame_t **frame, switch_io_flag_t flags,
...
SWITCH_DECLARE(switch_status_t) switch_core_session_fast_read_frame_from_jitterbuffer(switch_core_session_t *session, 
...
/*!
 \brief Return the number of consecutive frames with energy level < 41dB
 \param session the session for which to get the count
 \return uint32_t 0 if no silent frames, count if more than 100 consecutive frames with low energy
*/
...
SWITCH_DECLARE(void) switch_core_session_get_webrtc_counts(switch_core_session_t *session, uint32_t *read, uint32_t *insert,
                                                           uint32_t *hot_read, uint32_t *errors, uint32_t *skip_read, uint32_t *empty);
...
SWITCH_DECLARE(switch_status_t) switch_core_session_write_enc_frame(_In_ switch_core_session_t *session,
                                                                    switch_frame_t *frame,
                                                                    switch_io_flag_t flags,
                                                                    int stream_id,
                                                                    switch_time_t *timed);

SWITCH_DECLARE(switch_status_t) switch_core_session_enc_frame(_In_ switch_core_session_t *session,
                                                              switch_frame_t *frame, switch_io_flag_t flags,
                                                              int stream_id, switch_frame_t **ret_enc_frame);
...
/*! 
  \brief Get the latest interval ticks
  \param interval: interval in ms.
  \param ticks : output of tick count
  \return SWITCH_STATUS_SUCCESS after destruction
*/
SWITCH_DECLARE(switch_status_t) switch_core_timer_tick(uint32_t interval, size_t *ticks);
///\}

/*
 \brief Get the current time based on softtimer.
 \param now_us: output of time
 */
SWITCH_DECLARE(switch_status_t) switch_core_timer_now_us(switch_timer_t *timer, switch_time_t *now_us);
...
SWITCH_DECLARE(switch_status_t) switch_core_codec_copy(switch_codec_t *codec, switch_codec_t *new_codec, switch_memory_pool_t *pool, int channels, int bitrate);
...
SWITCH_DECLARE(void) switch_register_module_interfaces(switch_loadable_module_interface_t *module_interface);
...
/*! 
  \brief Register a global periodic logging handler
*/
SWITCH_DECLARE(void) switch_core_add_periodic_log_handler(_In_ const switch_periodic_log_t periodic_log);
/*!
 \brief Logs periodic stats - logs stats and fire an event that contains stats and rtp events
 \param [in] force - if set to false the event information is batched and may not be published right away
 \param [in] flush - if set to true and event exists it is published, no new stats are collected from the channel
*/
...
/*! 
  \brief Register a global logging filter: Can filter what needs to be logged
*/
typedef int (*log_check_t) (const char *key);
SWITCH_DECLARE(void) switch_core_set_log_filter_cb(switch_core_session_t *session, log_check_t log_check);

/*! 
  \brief Checks if a given key can be logged or not
*/
