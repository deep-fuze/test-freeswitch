...
*/
#define switch_channel_hangup(channel, hangup_cause) switch_channel_perform_hangup(channel, __FILE__, __SWITCH_FUNC__, __LINE__, hangup_cause, NULL)

#define switch_channel_hangup_with_cause(channel, hangup_cause, cause_str) switch_channel_perform_hangup(channel, __FILE__, __SWITCH_FUNC__, __LINE__, hangup_cause, cause_str)
/*!
...
SWITCH_DECLARE(int) switch_channel_set_running_stats(switch_channel_t *channel, const char* key, const char* value);
SWITCH_DECLARE(int) switch_channel_update_running_stats_by_idx(switch_channel_t *channel, int idx, const char* key, const char* value);
SWITCH_DECLARE(int) switch_channel_update_running_stats(switch_channel_t *channel, const char* key, const char* value);
...
/*
Fire call event for the channel. Function performs some internal batching, so the event may not be fired right away
 \param channel - channel for which to fire events
 \param force - fire event without batching
 \flush - fire all the batched data without generating new event
*/
