...
#if 0
		if (loopcount % 10 == 0) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "switch_ivr_gentones\n");
		}
		loopcount += 1;
#endif
...
SWITCH_DECLARE(switch_status_t) switch_ivr_play_file(switch_core_session_t *session, switch_file_handle_t *fh, const char *file, switch_input_args_t *args)
{
	...
	for (cur = 0; switch_channel_ready(channel) && !done && cur < argc; cur++) {
		...
		for (;;) {
...
#if 0
			if (loopcount % 10 == 0) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "switch_ivr_play_file\n");
			}
			loopcount += 1;
#endif
			...
			write_frame.seq = 0;
            write_frame.flags |= SFF_IVR_FRAME;
...
#if 0
		if (loopcount % 10 == 0) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "wait_for_silence\n");
		}
		loopcount += 1;
#endif
