...
#define PERIODIC_STATS_INTERVAL_MS (1 * 60 * 1000) //1 mins
...
static void *audio_bridge_thread(switch_thread_t *thread, void *obj)
{
    ...
    int pending_event = 0;  
    switch_time_t last_stat_report_time_ms = switch_micro_time_now() / 1000;
    int ticks_per_interval, ticks_per_stats_check, ticks = 0, check_ticks = 0, leadin_over = 0;
    int interval_ms, ticks_per_heartbeat, monitor_ticks = 0;
    ...
    switch_monitor_change_desc(tid, switch_core_get_monitor_index(session_a), "audio_bridge_thread");
    ...
    for (;;) {
        ...
        if (read_frame_count % 100 == 0) {
            if (switch_get_rtp_session_description(chan_a, description_tmp, MAX_DESCRIPTION_LEN)) {
                if (strcmp(description_tmp, description_a) != 0) {
                    strncpy(description_a, description_tmp, MAX_DESCRIPTION_LEN);
                    changed = SWITCH_TRUE;
                }
                if (switch_get_rtp_session_description(chan_b, description_tmp, MAX_DESCRIPTION_LEN)) {
                    if (strcmp(description_tmp, description_b) != 0) {
                        strncpy(description_b, description_tmp, MAX_DESCRIPTION_LEN);
                        changed = SWITCH_TRUE;
                    }
                    if (changed) {
                        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session_a), SWITCH_LOG_DEBUG, "bridge description %s <-> %s\n",
        ...
        if (++monitor_ticks >= ticks_per_heartbeat) {
            switch_monitor_alive(tid, switch_core_get_monitor_index(session_a));
            monitor_ticks = 0;
        }
        ...
        if (SWITCH_READ_ACCEPTABLE(status)) {
            ...
            if (read_frame->flags & SFF_RTP_EVENT) {
                skip = SWITCH_TRUE;
            } else if (read_frame->flags & SFF_RFC2833) {
                skip = SWITCH_TRUE;
            } else if (read_frame->flags & SFF_TIMEOUT && !(read_frame->flags & SFF_CNG)) {
                skip = SWITCH_TRUE;
            }

            if (!leadin_over) {
                switch_time_t now = switch_micro_time_now();
                if (now/1000 > last_stat_report_time_ms + STATS_LEADIN_TIME_MS) {
                    switch_clear_flag(read_frame, SFF_RTP_EVENT);
                    leadin_over = 1;
                }
            } else if ((read_frame->flags & SFF_RTP_EVENT) || pending_event) {
                switch_time_t now = switch_micro_time_now();
                if (now/1000 > last_stat_report_time_ms + MIN_STAT_REPORT_INTERVAL_MS) {
                    switch_bool_t do_event = SWITCH_TRUE;
                    switch_core_ioctl_stats(session_a, UPDATE_PERIODIC_STATS, &do_event);
                    switch_core_log_periodic(session_a, SWITCH_FALSE, SWITCH_FALSE);
                    last_stat_report_time_ms = now/1000;
                    pending_event = 0;
                } else {
                    pending_event = 1;
                }
                ...
                if (check_ticks++ >= ticks_per_stats_check) {
                ...
                if (ticks++ == ticks_per_interval) {
                    ...
                    switch_core_ioctl_stats(session_a, UPDATE_PERIODIC_STATS, &do_event);
                    switch_core_log_periodic(session_a, SWITCH_TRUE, SWITCH_FALSE);
                    switch_rtp_reset_rtp_stats(chan_a);
                    ticks = 0;
                }   
            }
...
#if 0
                /* fuze: we want to pass CN packets through */
...
static switch_status_t audio_bridge_on_exchange_media(switch_core_session_t *session)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "audio_bridge_on_exchange_media\n");

    if (bd) {
        switch_channel_set_private(channel, "_bridge_", NULL);
        if (bd->session == session && *bd->b_uuid) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "audio_bridge_on_exchange_media:audio_bridge_thread\n");
                                                                 ...
                                                                 void *peer_session_data)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "switch_ivr_multi_threaded_bridge\n");
    ...
    if (switch_channel_test_flag(peer_channel, CF_ANSWERED) || switch_channel_test_flag(peer_channel, CF_EARLY_MEDIA) ||
        ...
        if (switch_core_session_read_lock(peer_session) == SWITCH_STATUS_SUCCESS) {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "switch_ivr_multi_threaded_bridge:audio_bridge_thread\n");
