...
SWITCH_DECLARE(void) CoreSession::hangup(const char *cause)
{
	...
	this->begin_allow_threads();
    switch_channel_hangup_with_cause(channel, switch_channel_str2cause(cause), cause);
	this->end_allow_threads();
}
...
SWITCH_DECLARE(void) CoreSession::setDTMFCallback(PyObject *cbfunc, char *funcargs) {
