                                                       ...
                                                       int channels, int bitrate)
{
    ...
    switch_codec_settings_t *pSettings = NULL;
    ...
    if (channels) {
        codec_settings.channels = channels;
        codec_settings.bits_per_second = bitrate;
                                                       ...
                                                       const switch_codec_settings_t *codec_settings, switch_memory_pool_t *pool)
{
    ...
    for (iptr = codec_interface->implementations; iptr; iptr = iptr->next) {
        ...
        //int channels_ = !strcasecmp(codec_name, "opus") ? 1 : channels;
        int channels_ = channels;
        if ((!rate || rate == crate) && (!bitrate || !iptr->bits_per_second || bitrate == (uint32_t)iptr->bits_per_second) &&
