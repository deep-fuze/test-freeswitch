...
static const char* rtp_stat_name[RTP_MAX_STAT] =
{
    [RTP_CURRENT_TIME] = "current_local_time",
    [RTP_EVENT_NAME] = "event_name",
    [RTCP_MAX_DRIFT] = "rtp_audio_rtcp_max_drift_ms",
    [RTCP_LAST_DRIFT] = "rtp_audio_rtcp_last_drift_ms",
    [RTCP_MAX_RTT] = "rtp_audio_rtcp_max_rtt_ms",
    [RTCP_AVG_RTT] = "rtp_audio_rtcp_avg_rtt_ms",
    [RTCP_LAST_RTT] = "rtp_audio_rtcp_last_rtt_ms",
    [RTP_SKIP_COUNT] = "rtp_audio_in_skip_packet_count",
    [RTP_TOTAL_LOST] = "rtp_audio_rtcp_total_lost_from_client",
    [RTP_PERIOD_SKIP_COUNT] = "rtp_audio_in_period_skip_packet_count",
    [RTP_PERIOD_LOST] = "rtp_audio_in_period_lost_from_client",
    [RTP_PERIOD_TOTAL] = "rtp_audio_in_period_total",
    [RTP_PACKET_COUNT] = "rtp_audio_in_packet_count",
    ...
    [RTP_RECV_CNT] = "event-recv-cnt",
    [RTP_SEND_CNT] = "event-send-cnt",
    [RTP_MOS] = "event-mos",
    [RTP_R] = "event-r",
...
{
    [RTP_EVENT_PERIODIC] = "PERIODIC",
    [RTP_EVENT_HIGH_DRIFT] = "HIGH_DRIFT",
    [RTP_EVENT_HIGH_RTT] = "HIGH_RTT",
    [RTP_EVENT_HIGH_CONSECUTIVE_PACKET_LOSS] = "HIGH_PACKET_LOSS",
    [RTP_EVENT_LONG_JITTER_BUFFER] = "LONG_JITTER_BUFFER",
};

SWITCH_DECLARE(void) switch_core_media_set_email_and_phone(switch_core_session_t *session, const char *r_sdp);
SWITCH_DECLARE(void) switch_core_media_set_session_name(switch_core_session_t *session, const char *r_sdp);
...
#define add_signed_stat(_i, _s)                                             \
    switch_snprintf(var_name, sizeof(var_name), "rtp_%s_%s", switch_str_nil(prefix), _s) ; \
    switch_snprintf(var_val, sizeof(var_val), "%d", _i); \
    switch_channel_set_variable(channel, var_name, var_val)
...
static void set_stats(switch_core_session_t *session, switch_media_type_t type, const char *prefix)
{
    ...
    stats = switch_core_media_get_stats(session, type, NULL);
    ...
    if (stats) {
        ...
        if (switch_rtp_get_webrtc_neteq(engine->rtp_session)) {
            ...
            add_stat(stats->inbound.skip_packet_count, "in_skip_packet_count");
            add_stat(stats->inbound.jb_max_len, "jb_max_qlen");
            add_stat(stats->inbound.jb_lost_count, "jb_lost_frames");
            add_stat(stats->inbound.jb_toolate_drop_count, "jb_toolate_dropped_frames");
            #ifdef _USE_NEW_JB_
            add_stat(stats->inbound.jb_overflow_drop_count, "jb_overflow_dropped_frames");
            add_stat(stats->inbound.jb_total_outoforder_count, "jb_out_of_order_frames");
            add_stat(stats->inbound.jb_exhaustion_count, "jb_exhaustion_count");
            add_stat(stats->inbound.jb_max_drift, "jb_max_drift");
            add_stat(stats->inbound.jb_average_drift, "jb_avg_drift");
            #endif
        }
        ...
        add_stat((stats->rtcp.jitter * 1000) / samples_per_second, "rtcp_jitter_ms");
        add_stat((stats->rtcp.max_jitter * 1000) / samples_per_second, "rtcp_max_jitter_ms");
        add_stat(stats->rtcp.last_drift, "rtcp_last_drift_ms");
        add_stat(stats->rtcp.max_drift, "rtcp_max_drift_ms");
        add_stat(stats->rtcp.last_rtt, "rtcp_last_rtt_ms");
        add_stat(stats->rtcp.avg_rtt, "rtcp_avg_rtt_ms");
        add_stat(stats->rtcp.max_rtt, "rtcp_max_rtt_ms");
        add_signed_stat(stats->cumulative_lost, "rtcp_total_lost_from_client");
...
static void mask_rtp_periodic_stats(switch_core_session_t *session, switch_media_type_t type)
{
    rtp_periodic_stat_t idx;
    switch_channel_t *channel = switch_core_session_get_channel(session);
    switch_rtp_stats_t *stats = switch_core_media_get_stats(session, type, NULL);

    if (stats) {
        for (idx = 0; idx < RTP_MAX_STAT; idx++) {
            if(stats->rtp_stat_global_index[idx]) {
                switch_mask_running_stat_by_idx(channel, stats->rtp_stat_global_index[idx]);
            }
        }
    }
}

static void set_periodic_stats_value(switch_core_session_t *session, rtp_periodic_stat_t stat, const char* value, switch_media_type_t type)
{
    switch_rtp_stats_t *stats = switch_core_media_get_stats(session, type, NULL);
    switch_channel_t *channel = switch_core_session_get_channel(session);

    if (stats &&
          switch_channel_update_running_stats_by_idx (channel, stats->rtp_stat_global_index[stat], rtp_stat_name[stat], value) < 0) {
        stats->rtp_stat_global_index[stat] = switch_channel_set_running_stats(channel, rtp_stat_name[stat], value);
    }
}

static void switch_core_media_set_rtp_event(switch_core_session_t *session, rtp_events_t event, switch_media_type_t type)
{
    switch_media_handle_t *smh;
    switch_rtp_engine_t *engine;
    ...
    if (engine) {
        switch_rtp_set_event(engine->rtp_session, event);
    }
}

static void set_periodic_stats(switch_core_session_t *session, switch_bool_t event_trigger, switch_media_type_t type)
{
    switch_rtp_stats_t *stats = switch_core_media_get_stats(session, type, NULL);
    switch_channel_t *channel = switch_core_session_get_channel(session);
    switch_media_handle_t *smh;
    ...
    if (!stats || !engine) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Error: Failed to set stats. Invalid params: %lx %lx\n",
    ...
    if ((vval = switch_channel_get_variable(channel, "conference_id"))) {
    ...
    if (stats->last_event) {
        for (event = 1; event < RTP_EVENT_MAX && len < sizeof(value); event <<= 1) {
            if ((stats->last_event & event)) {
                len += snprintf(value + len, sizeof(value) - len, "%s%s", (len ? " | " : ""),
                                rtp_event_name[event]);
            }
        }
    ...
    if (len)
        set_periodic_stats_value(session, RTP_EVENT_NAME, value, type);

    if (event_trigger == SWITCH_FALSE || (event & RTP_EVENT_HIGH_DRIFT)) {
        snprintf(value, sizeof(value), "%u", stats->rtcp.max_drift);
        set_periodic_stats_value(session, RTCP_MAX_DRIFT, value, type);

        snprintf(value, sizeof(value), "%u", stats->rtcp.last_drift);
        set_periodic_stats_value(session, RTCP_LAST_DRIFT, value, type);

        snprintf(value, sizeof(value), "%u", (stats->rtcp.jitter * 1000) / samples_per_second);
        set_periodic_stats_value(session, RTP_JITTER, value, type);

        snprintf(value, sizeof(value), "%u", (stats->rtcp.max_jitter * 1000) / samples_per_second);
        set_periodic_stats_value(session, RTP_MAX_JITTER, value, type);
    }

    if (event_trigger == SWITCH_FALSE || (event & RTP_EVENT_LONG_JITTER_BUFFER)) {
        snprintf(value, sizeof(value), "%u", stats->jb_cur_size);
        set_periodic_stats_value(session, RTP_CUR_JB_SIZE, value, type);
    }

    if (event_trigger == SWITCH_FALSE || (event & RTP_EVENT_HIGH_RTT)) {
        snprintf(value, sizeof(value), "%u", stats->rtcp.max_rtt);
        set_periodic_stats_value(session, RTCP_MAX_RTT, value, type);

        snprintf(value, sizeof(value), "%u", stats->rtcp.avg_rtt);
        set_periodic_stats_value(session, RTCP_AVG_RTT, value, type);

        snprintf(value, sizeof(value), "%u", stats->rtcp.last_rtt);
        set_periodic_stats_value(session, RTCP_LAST_RTT, value, type);
    }
    ...
    if (event_trigger == SWITCH_FALSE || (event & RTP_EVENT_HIGH_CONSECUTIVE_PACKET_LOSS) ||
        ...
        if (switch_rtp_get_webrtc_neteq(engine->rtp_session)) {
            ...
            set_periodic_stats_value(session, RTP_PERIOD_SKIP_COUNT, value, type);

            snprintf(value, sizeof(value), "%lu", stats->inbound.skip_packet_count);
            set_periodic_stats_value(session, RTP_SKIP_COUNT, value, type);
        }

        snprintf(value, sizeof(value), "%d", stats->period_lost_count);
        set_periodic_stats_value(session, RTP_PERIOD_LOST, value, type);

        snprintf(value, sizeof(value), "%d", stats->cumulative_lost);
        set_periodic_stats_value(session, RTP_TOTAL_LOST, value, type);

        snprintf(value, sizeof(value), "%u", stats->last_period_received);
        set_periodic_stats_value(session, RTP_PERIOD_TOTAL, value, type);
...
SWITCH_DECLARE(switch_status_t) switch_core_media_stats_ioctl(switch_core_session_t *session, int cmd, void *arg, switch_media_type_t type)
{
    switch_media_handle_t *smh;
    switch_rtp_engine_t *engine;
    ...
    engine = &smh->engines[SWITCH_MEDIA_TYPE_AUDIO];
    ...
    switch (cmd) {
        case SET_JB_SIZE:
        {
            if (arg) {
                switch_rtp_stats_t *stats = switch_core_media_get_stats(session, type, NULL);
                stats->jb_cur_size = *(int *) arg;
                ret = SWITCH_STATUS_SUCCESS;
            }
        }
        break;

        case SET_EVENT_LONG_JB:
            switch_core_media_set_rtp_event(session, RTP_EVENT_LONG_JITTER_BUFFER, type);
            ret = SWITCH_STATUS_SUCCESS;
            break;

        case SET_EVENT_HIGH_CONSECUTIVE_PACKET_LOSS:
            switch_core_media_set_rtp_event(session, RTP_EVENT_HIGH_CONSECUTIVE_PACKET_LOSS, type);
            ret = SWITCH_STATUS_SUCCESS;
            break;

        case UPDATE_PERIODIC_STATS:
        {
            if (arg) {
                if (*((switch_bool_t *) arg)== SWITCH_FALSE)
                    switch_core_media_set_rtp_event(session, RTP_EVENT_PERIODIC, type);

                if (engine->rtp_session) {
                    set_periodic_stats(session, *((switch_bool_t *) arg), type);
                    ret = SWITCH_STATUS_SUCCESS;
                }
            }
        }
        ...
        case GET_RTCP_INTERVAL:
            if (arg) {
                if (engine->rtp_session) {
                    *((int *) arg) = switch_rtp_get_rtcp_interval(engine->rtp_session);
                }
            }
            break;
        ...
        case SET_SEND_RTCP:
            if (arg) {
                if (engine->rtp_session) {
                    switch_rtp_set_send_rtcp(engine->rtp_session, *(int *)arg);
                }
            }
            break;

        case SET_ACTIVE_TALKER_FLAG:
            if (arg) {
                if (engine->rtp_session) {
                    switch_rtp_set_been_active_talker(engine->rtp_session, *(int *)arg);
                }
            }
            break;
        ...
        case SET_CHOP_EVENT:
            if (arg) {
                if (engine->rtp_session) {
                    switch_rtp_stats_t *stats = switch_core_media_get_stats(session, type, NULL);
                    if (stats) {
                        uint16_t chop_count = *(uint16_t *)arg;
                        uint16_t logn_floor = 0;
                        while (chop_count >= (1 << (logn_floor + 1))) {
                            logn_floor++;
                            if (logn_floor > sizeof(stats->jb_period_chop_events) - 1) {
                                logn_floor = sizeof(stats->jb_period_chop_events) - 1;
                                break;
                            }
                        }

                        stats->jb_period_chop_events[logn_floor]++;
                    }
                }
            }
            break;

        case SET_LOST_PERCENT:
            if (arg) {
                if (engine) {
                    switch_rtp_stats_t *stats = switch_core_media_get_stats(session, type, NULL);
                    if (stats) {
                        stats->jb_period_lost_percent = *(int *)arg;
                    }
                }
            }
            break;
                                                             ...
                                                             switch_io_flag_t flags, int stream_id, switch_media_type_t type)
{
    ...
    while (smh->media_flags[SCMF_RUNNING] && engine->read_frame.datalen == 0) {
        switch_time_t current_interval = switch_micro_time_now()/1000;
        ...
        if (switch_rtp_get_webrtc_neteq(engine->rtp_session)) {
            if (status == SWITCH_STATUS_BREAK) {
                switch_time_t interval_now = switch_micro_time_now()/1000;
                if(interval_now > current_interval + (switch_time_t)5) {
                    break;
                }
            }
        }
        ...
        if (engine->read_frame.datalen > 0) {
            ...
            if (!switch_test_flag((&engine->read_frame), SFF_CNG)) {
                ...
                if (!engine->reset_codec &&
                    ...
                    engine->read_frame.payload != 72 && /* RTCP Sender Report */
    ...
    if (switch_channel_test_flag(smh->session->channel, CF_REINVITE)) {
        ...
        if (engine->rtp_session && ((val = switch_channel_get_variable(smh->session->channel,
            ...
            if (!strcasecmp(val, "passthru")) {
                ...
                switch_rtp_activate_rtcp(engine->rtp_session, NULL, -1, remote_rtcp_port, engine->rtcp_mux > 0);
            } else {
                ...
                switch_rtp_activate_rtcp(engine->rtp_session, NULL, interval, remote_rtcp_port, engine->rtcp_mux > 0);
            }
        }
...
{
    uint8_t match = 0;
    switch_payload_t best_te = 0, te = 0, cng_pt8 = 0, cng_pt16 = 0;
    sdp_media_t *m;
    sdp_list_t *l;
    sdp_attribute_t *attr;
    ...
    for (attr = sdp->sdp_attributes; attr; attr = attr->a_next) {
        ...
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                          "SDP attribute %s : %s", attr->a_name, attr->a_value);
        ...
        if (!strcasecmp(attr->a_name, "sendonly")) {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
                              "Unknown SDP attribute %s : %s", attr->a_name, attr->a_value);
        }
    }
    ...
    for (l = sdp->sdp_emails; l; l = l->l_next) {
        if (l->l_text) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                              SWITCH_LOG_INFO, "EMAIL from SDP: %s\n",
                              l->l_text);
            if (strlen(session->email) == 0) {
                strncpy(session->email, l->l_text, SWITCH_CORE_EMAIL_LEN);
    ...
    /* set phones */
    for (l = sdp->sdp_phones; l; l = l->l_next) {
        if (l->l_text) {
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session),
                              SWITCH_LOG_INFO, "PHONE from SDP: %s\n",
                              l->l_text);
            if (strlen(session->phone) == 0) {
                strncpy(session->phone, l->l_text, SWITCH_CORE_PHONE_LEN);
                switch_set_rtp_session_phone(a_engine->rtp_session, session->phone);
                break;
            }
        }
    }
    ...
    for (m = sdp->sdp_media; m; m = m->m_next) {
        ...
        if (got_udptl && m->m_type == sdp_media_image && m->m_port) {
            ...
            for (map = m->m_rtpmaps; map; map = map->rm_next) {
                ...
                switch_bool_t fuze_app = SWITCH_FALSE;
#ifdef FUZE_APP_CN
                fuze_app = (switch_channel_get_variable(session->channel, "fuze_app") != NULL);
#endif
                ...
                if ((!switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG) || fuze_app) &&
                    (!cng_pt8 || !cng_pt16) && !strcasecmp(rm_encoding, "CN")) {

                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "enc:%s/%lu pt=%d\n", rm_encoding, map->rm_rate, map->rm_pt);
                    if (map->rm_rate == 8000) {
                        cng_pt8 = (switch_payload_t) map->rm_pt;
                    } else if (map->rm_rate == 16000) {
                        cng_pt16 = (switch_payload_t) map->rm_pt;
                    }
                    continue;
                }
            ...
            if (match) {
                ...
                if (a_engine->rtp_session) {
                    int cng_pt = cng_pt8;
                    if (a_engine->cur_payload_map->recv_pt == G722_PT || a_engine->cur_payload_map->rm_rate == 16000) {
                        cng_pt = cng_pt16;
                    }
                    if (cng_pt) {
    ...
    if (a_engine->cur_payload_map->rm_rate == 8000 && a_engine->cur_payload_map->recv_pt != G722_PT) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "setting cng_pt to pt=%d\n", cng_pt8);
        smh->mparams->cng_pt = cng_pt8;
    } else {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "setting cng_pt to pt=%d\n", cng_pt16);
        smh->mparams->cng_pt = cng_pt16;
    }
    if (a_engine->rtp_session && smh->mparams->cng_pt) {
        switch_rtp_set_cng_pt(a_engine->rtp_session, smh->mparams->cng_pt);
    }

    if (smh->mparams->cng_pt) {
        switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Set comfort noise payload to rate=%lu pt=%u\n", 
                          a_engine->cur_payload_map->rm_rate, smh->mparams->cng_pt);
    }
...
static void check_dtls_reinvite(switch_core_session_t *session, switch_rtp_engine_t *engine)
{
    if (switch_channel_test_flag(session->channel, CF_REINVITE)) {
        ...
        if (!zstr(engine->local_dtls_fingerprint.str) && switch_rtp_has_dtls() && dtls_ok(session)) {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "dtls type %s\n", dtype == DTLS_TYPE_CLIENT ? "client" : "server");
#endif
...
//?
SWITCH_DECLARE(switch_status_t) switch_core_media_activate_rtp(switch_core_session_t *session, void *tbase)

{
    ...
    if (switch_channel_up(session->channel)) {
        ...
        a_engine->rtp_session = switch_rtp_new(tbase,
                                  a_engine->local_sdp_ip,
                                  a_engine->local_sdp_port,
                                  a_engine->cur_payload_map->remote_sdp_ip,
                                  a_engine->cur_payload_map->remote_sdp_port,
    ...
    if (switch_rtp_ready(a_engine->rtp_session)) {
        ...
        if ((val = switch_channel_get_variable(session->channel, "rtcp_audio_interval_msec")) || (val = smh->mparams->rtcp_audio_interval_msec)) {
            ...
            if (!strcasecmp(val, "passthru")) {
                ...
                switch_rtp_activate_rtcp(a_engine->rtp_session, tbase, -1, remote_rtcp_port, a_engine->rtcp_mux > 0);
            } else {
                int interval = atoi(val);
                if (interval < 100 || interval > 500000) {
                ...
                switch_rtp_activate_rtcp(a_engine->rtp_session, tbase, interval, remote_rtcp_port, a_engine->rtcp_mux > 0);
        ...
        if (!zstr(a_engine->local_dtls_fingerprint.str) && switch_rtp_has_dtls() && dtls_ok(smh->session)) {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(smh->session), SWITCH_LOG_INFO, "dtls type %s\n", dtype == DTLS_TYPE_CLIENT ? "client" : "server");
#endif
        ...
        if (!fuze_app &&
            (switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG) ||
    ...
    video:
        ...
        if (switch_channel_test_flag(session->channel, CF_VIDEO_POSSIBLE) && v_engine->cur_payload_map->rm_encoding && v_engine->cur_payload_map->remote_sdp_port) {
            ...
            v_engine->rtp_session = switch_rtp_new(tbase,
                                    a_engine->local_sdp_ip,
                                    v_engine->local_sdp_port,
                                    v_engine->cur_payload_map->remote_sdp_ip,
            ...
            if (switch_rtp_ready(v_engine->rtp_session)) {
                ...
                if ((val = switch_channel_get_variable(session->channel, "rtcp_video_interval_msec")) || (val = smh->mparams->rtcp_video_interval_msec)) {
                    ...
                    if (!strcasecmp(val, "passthru")) {
                        ...
                        switch_rtp_activate_rtcp(v_engine->rtp_session, NULL, -1, remote_port, v_engine->rtcp_mux > 0);
                    } else {
                        int interval = atoi(val);
                        if (interval < 100 || interval > 500000) {
                            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
                        ...
                        switch_rtp_activate_rtcp(v_engine->rtp_session, NULL, interval, remote_port, v_engine->rtcp_mux > 0);
...
SWITCH_DECLARE(void) switch_core_media_gen_local_sdp(switch_core_session_t *session, switch_sdp_type_t sdp_type, const char *ip, switch_port_t port, const char *sr, int force)
{
    ...
    if ((sname = switch_channel_get_variable(session->channel, "fuze_app"))) {
        switch_snprintf(sessionname, 128, "%s:%s", username, sname);
    } else {
        switch_snprintf(sessionname, 128, "%s", username);
    }
    ...
    switch_snprintf(buf, SDPBUFLEN,
                    ...
                    "t=0 0\n"
                    "%s",
                    username, smh->owner_id, smh->session_id, family, ip, sessionname, family, ip, srbuf);
    ...
    if (strlen(session->phone) > 0) {
        switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "p=%s\n", session->phone);
    }
    ...
    if (a_engine->codec_negotiated) {
        ...
        if ((!switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG) || fuze_app) && smh->mparams->cng_pt && use_cng) {
        ...
        if (switch_media_handle_test_media_flag(smh, SCMF_SUPPRESS_CNG)) {
            ...
            if (fuze_app && smh->mparams->cng_pt && use_cng) {
                int rate = a_engine->cur_payload_map->rm_rate;
                if (a_engine->cur_payload_map->agreed_pt == G722_PT) {
                    rate = rate * 2;
                }
                switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=rtpmap:%d CN/%lu\n", smh->mparams->cng_pt, rate);
            }
    ...
    if ((rfc6464 = switch_channel_get_variable(session->channel, "rfc6464"))) {
        switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=extmap:%s\n", rfc6464);
    }

    {
        const char *direction;
        if ((direction = switch_channel_get_variable(session->channel, "peer-direction"))) {
            if (!strncasecmp(direction, "active", 6)) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Direction of peer is active\n");
                switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=setup:passive\n");
                // switch_snprintf(buf + strlen(buf), SDPBUFLEN - strlen(buf), "a=direction:passive\n");
            } else {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Direction of peer is %s\n", direction);
            }
        }
    }
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "fuze setting local sdp\n");
...
SWITCH_DECLARE(void) switch_core_media_absorb_sdp(switch_core_session_t *session)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "fuze switch_core_media_absorb_sdp\n");
...
SWITCH_DECLARE(void) switch_core_media_set_udptl_image_sdp(switch_core_session_t *session, switch_t38_options_t *t38_options, int insist)
{
    ...
    switch_snprintf(buf, sizeof(buf),
                    "v=0\n"
                    "o=%s %010u %010u IN %s %s\n"
                    "s=%s\n" "c=IN %s %s\n" "t=0 0\n", username, smh->owner_id, smh->session_id, family, ip, sessionname, family, ip);
...
SWITCH_DECLARE(void) switch_core_media_patch_sdp(switch_core_session_t *session)
{
    ...
    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "fuze switch_core_media_patch_sdp\n");
        ...
        if (a_engine->local_sdp_ip && !strncmp("c=IN IP", p, 7)) {
            ...
            if (!has_video) {
                ...
                if (switch_channel_media_ready(session->channel) && !switch_rtp_ready(v_engine->rtp_session)) {
                    ...
                    switch_core_media_activate_rtp(session, NULL);
                }
            }
            ...
            if (switch_rtp_ready(a_engine->rtp_session)) {
                ...
                if (msg->string_arg) {
                    ...
                    if (!strcasecmp(msg->string_arg, "pause")) {
                        ...
                        goto end;
                    } else if (!strncasecmp(msg->string_arg, "use_webrtc_neteq=", strlen("use_webrtc_neteq="))) {
                        s = msg->string_arg + strlen("use_webrtc_neteq=");
                        if (s && !strcmp(s, "true")) {
                            switch_rtp_set_webrtc_neteq(a_engine->rtp_session, SWITCH_TRUE);
                            switch_channel_set_variable(session->channel, "use_webrtc_neteq", "true");
                            switch_channel_set_variable(session->channel, "jitterbuffer_impl", "neteq");
                            switch_channel_clear_flag(session->channel, CF_JITTERBUFFER);
                            break;
                        }
                    }
    ...
    case SWITCH_MESSAGE_INDICATE_NOMEDIA:
        {
            ...
            if (!smh->mparams->local_sdp_str) {
                switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "fuze calling switch_core_media_absorb_sdp\n");
...
/* switch_channel_get_variable(session->channel, "fuze_app") */
SWITCH_DECLARE(void) switch_core_media_set_session_name(switch_core_session_t *session, const char *r_sdp)
{
    sdp_parser_t *parser;
    sdp_session_t *sdp;
    ...
    for (m = sdp->sdp_media; m; m = m->m_next) {
        for (map = m->m_rtpmaps; map; map = map->rm_next) {
            ...
            if (!strcasecmp(rm_encoding, "CN")) {
                smh->mparams->cng_pt = (switch_payload_t) map->rm_pt;
                sdp_parser_free(parser);
                return;
            }
        }
...
/* xxx */
SWITCH_DECLARE(void) switch_core_media_set_extmap_string(switch_core_session_t *session, const char *r_sdp)
{
    sdp_parser_t *parser;
    sdp_session_t *sdp;
    switch_media_handle_t *smh;
...
// MQT-6348: Disable for now
                        ...
                        if (strstr(attr->a_value, "ssrc-audio-level")) {
                            ...
                            switch_channel_set_variable(session->channel, "rfc6464", attr->a_value);
                            if ((uuid = switch_channel_get_partner_uuid(session->channel))
                                && (other_session = switch_core_session_locate(uuid))) {
                                switch_channel_t *other_channel = switch_core_session_get_channel(other_session);
                                if (other_channel) {
                                    switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(other_session), SWITCH_LOG_INFO,
                                                      "setting rfc6464 for this session as well %s\n", attr->a_value);
                                    switch_channel_set_variable(other_channel, "rfc6464", attr->a_value);
                                }
                                switch_core_session_rwunlock(other_session);
                            }
                        }
...
SWITCH_DECLARE (void) switch_core_media_recover_session(switch_core_session_t *session)
{
    ...
    if (switch_core_media_activate_rtp(session, NULL) != SWITCH_STATUS_SUCCESS) {
        return;
    }
