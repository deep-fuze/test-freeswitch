...
static core_api_command_t session_api_sub_commands[] = {
    {"list", (void_fn_t) NULL, "list", ""},
    {"rtp_detail", (void_fn_t) NULL, "rtp_detail", "<uuid|all|inactive>"},
};

static core_api_command_t monitor_api_sub_commands[] = {
    {"threads", (void_fn_t) NULL, "threads", "<all|stuck>"},
};

SWITCH_STANDARD_API(session_api_main);
SWITCH_STANDARD_API(monitor_api_main);
static switch_status_t session_api_list(switch_stream_handle_t *stream, int argc, char **argv);
static switch_status_t session_api_rtp_detail(switch_stream_handle_t *stream, int argc, char **argv);

#define SESSFUNCAPISIZE (sizeof(session_api_sub_commands)/sizeof(session_api_sub_commands[0]))
#define MONITORFUNCAPISIZE (sizeof(monitor_api_sub_commands)/sizeof(monitor_api_sub_commands[0]))
...
SWITCH_DECLARE(jb_t *) switch_core_session_get_jb(switch_core_session_t *session, switch_media_type_t type)
{
...
SWITCH_DECLARE(void) switch_core_session_change_thread(switch_core_session_t *session)
{
...
static int session_can_log_key(const char *key)
{
        int i;
...
static void *SWITCH_THREAD_FUNC switch_core_session_thread(switch_thread_t *thread, void *obj)
{
    ...
    session->thread_id = switch_thread_self();
    switch_monitor_start(session->thread_id, "switch_core_session_thread", 
                                    DEFAULT_MAX_INACTIVE_MS, &session->monitor_index);
    mindex = session->monitor_index;

    /* Set the log filter callback, that gets called to filter the logs
           at the end of the session. This is default call back. Individual
       apps can choose to override if needed. */
    switch_core_set_log_filter_cb(session, session_can_log_key);
    ...
    switch_monitor_stop(switch_thread_self(), mindex);
...
//fuze
    session->suppress_lost = 0;
    session->audio_level_low_count = 0;

    session->period_start = switch_micro_time_now() / 1000;
    session->pkt_errors = 0;

    session->insert = 0;
    session->read = 0;
    session->skip_insert = 0;
    session->skip_read = 0;
...
{
    if (session && session->log_check)
        return session->log_check(key); 
...
{
    if (session && session->neteq_inst) {
        WebRtcNetEQ_Destroy(session->neteq_inst);
    }
}
...
SWITCH_DECLARE(switch_status_t) switch_core_register_api(switch_memory_pool_t *pool)
{
    ...
    switch_status_t status = SWITCH_STATUS_SUCCESS;
    char cmd_str[256];
    switch_loadable_module_interface_t *m_interface;
    switch_loadable_module_interface_t **module_interface;

    /*
     * Lets create a dummy module for session so that we can create api interface.
     */
    m_interface = switch_loadable_module_create_module_interface(pool, "session");
    if (!m_interface) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Failed to create a dummy module session. Session API will not be available.\n");
        return SWITCH_STATUS_FALSE;
    }

    module_interface = &m_interface;

    /* build api interface help ".syntax" field string */
    p = strdup("");
    for (i = 0; i < SESSFUNCAPISIZE; i++) {
        nl = strlen(session_api_sub_commands[i].pcommand) + strlen(session_api_sub_commands[i].psyntax) + 5;

        switch_snprintf(cmd_str, sizeof(cmd_str), "add conference ::conference::list_conferences %s", session_api_sub_commands[i].pcommand);
        ...
        if (tmp != NULL) {
            p = tmp;
            strcat(p, "\t\t");
            strcat(p, session_api_sub_commands[i].pcommand);
            if (!zstr(session_api_sub_commands[i].psyntax)) {
                strcat(p, " ");
                strcat(p, session_api_sub_commands[i].psyntax);
            }
            if (i < SESSFUNCAPISIZE - 1) {
                strcat(p, "\n");
            }
        } else {
    ...
    SWITCH_ADD_API(api_interface, "session", "Core commands", session_api_main, p);
 
    /* build api interface help ".syntax" field string */
    p = strdup("");
    for (i = 0; i < MONITORFUNCAPISIZE; i++) {
        nl = strlen(monitor_api_sub_commands[i].pcommand) + strlen(monitor_api_sub_commands[i].psyntax) + 5;

        switch_snprintf(cmd_str, sizeof(cmd_str), "add conference ::conference::list_conferences %s", monitor_api_sub_commands[i].pcommand);
        ...
        if (tmp != NULL) {
            p = tmp;
            strcat(p, "\t\t");
            strcat(p, monitor_api_sub_commands[i].pcommand);
            if (!zstr(monitor_api_sub_commands[i].psyntax)) {
                strcat(p, " ");
                strcat(p, monitor_api_sub_commands[i].psyntax);
            }
            if (i < MONITORFUNCAPISIZE - 1) {
                strcat(p, "\n");
            }
        } else {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't realloc\n");
    ...
    SWITCH_ADD_API(api_interface, "monitor", "Core commands", monitor_api_main, p);
 
    switch_register_module_interfaces(m_interface);
    /*
     * memory allocated for p will be active for the duration of process. so no need to free it.
     */ 
...
SWITCH_STANDARD_API(session_api_main)
{
    ...
    if (argc && argv[0]) {
        if (strcasecmp(argv[0], "list") == 0) {
            return session_api_list(stream, argc, argv);
        } else if (strcasecmp(argv[0], "rtp_detail") == 0) {
            return session_api_rtp_detail(stream, argc, argv);
        } 
    } 
        
    for (i = 0; i < SESSFUNCAPISIZE; i++) {
        stream->write_function(stream, "session %s %s\n", 
                session_api_sub_commands[i].pcommand, session_api_sub_commands[i].psyntax);
    }
...
SWITCH_STANDARD_API(monitor_api_main)
{
    ...
    if (argc && argv[0]) {
        if (strcasecmp(argv[0], "threads") == 0) {
            return switch_monitor_report(stream, argc, argv);
        } 
    } 
    
    for (i = 0; i < MONITORFUNCAPISIZE; i++) {
        stream->write_function(stream, "monitor %s %s\n", 
                monitor_api_sub_commands[i].pcommand, monitor_api_sub_commands[i].psyntax);
    }
...
static switch_status_t session_api_rtp_detail(switch_stream_handle_t *stream, int argc, char **argv)
{
    ...
    switch_core_session_t *session;
    int first_hi = 1;
    switch_rtp_stats_t rtp_stats;
    char *ch_type, *call_id, *caller_id;

    if (argc <= 1 || !argv[1]) {
        stream->write_function(stream, "session rtp_detail <uuid|all|inactive>\n"); 
        return SWITCH_STATUS_SUCCESS;
    }
    ...
    for (hi = switch_core_hash_first(session_manager.session_table); hi; hi = switch_core_hash_next(&hi)) {
        ...
        if (val) {
            session = (switch_core_session_t *) val;
            if (switch_core_ioctl_stats(session, GET_RTP_STATS, &rtp_stats) == SWITCH_STATUS_FALSE)
                continue;

            if (strcasecmp(argv[1], "all") == 0 ||
                    strcasecmp(argv[1], session->uuid_str) == 0 ||
                    (strcasecmp(argv[1], "inactive") == 0 && 
                     session->last_rtp_read_pkt_count == rtp_stats.inbound.media_packet_count)) {
                
                if (first_hi) {
                    stream->write_function(stream, "uuid, call_id, caller_id, channel_type, inbound_data_pkts\n");
                    first_hi = 0;
                }

                if (session->channel) {
                    if (!(ch_type = (char *) switch_channel_get_variable(session->channel, "channel_type"))) {
                        ch_type = "-";
                    } 

                    if (!(call_id = (char *) switch_channel_get_variable(session->channel, "sip_call_id"))) {
                        call_id = "-";
                    } 

                    if (!(caller_id = (char *) switch_channel_get_variable(session->channel, "caller_id_number"))) {
                        caller_id = "-";
                    } 
                ...
                stream->write_function(stream, "%s, %s, %s, %s, %lu\n", session->uuid_str, call_id, caller_id, ch_type, rtp_stats.inbound.media_packet_count);
            } 
    ...
    if (first_hi)
        stream->write_function(stream, "0 sessions.\n");
