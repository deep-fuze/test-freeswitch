...
SWITCH_DECLARE(void) switch_sockaddr_set_port(switch_sockaddr_t *sa, switch_port_t port)
{
	apr_sockaddr_set_port(sa, port);
}
...
SWITCH_DECLARE(switch_status_t) switch_get_sockaddr_details(switch_sockaddr_t *saddr, int *family, void *sin, int max_sin_size,
                                                                                        void *sin6, int max_sin6_size)
{
    if (!saddr)
        return SWITCH_STATUS_FALSE;

    *family = saddr->family;
    if(*family == APR_INET && max_sin_size >= sizeof(struct sockaddr_in)) {
        *(struct sockaddr_in *) sin = saddr->sa.sin;
    } else if (*family == APR_INET6 && max_sin6_size >= sizeof(struct sockaddr_in6)) {
        *(struct sockaddr_in6 *) sin6 = saddr->sa.sin6;
    } else {
        return SWITCH_STATUS_FALSE;
    }
...
SWITCH_DECLARE(void) switch_set_sockaddr_v4(switch_sockaddr_t *saddr, uint32_t addr, uint16_t port)
{
    saddr->sa.sin.sin_addr.s_addr = htonl(addr);
    saddr->salen = sizeof(saddr->sa);
    apr_sockaddr_vars_set(saddr, APR_INET, port);
}

SWITCH_DECLARE(switch_status_t) switch_get_sockaddr_v4(switch_sockaddr_t *saddr, uint32_t *addr, uint16_t *port) 
{
    if (!saddr || saddr->family != APR_INET)
        return SWITCH_STATUS_FALSE;

    *addr = ntohl(saddr->sa.sin.sin_addr.s_addr);
    *port = ntohs(saddr->sa.sin.sin_port);
